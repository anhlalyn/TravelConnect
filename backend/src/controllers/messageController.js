...
const ensureMessageSchema = async () => {
  const [rows] = await db.query(
    "SHOW COLUMNS FROM tin_nhan LIKE 'loai_tin_nhan'",
  );
...
exports.getMessagesByRoom = async (req, res) => {
  ...
  try {
    ...
    if (!hasAccess) {
      return res
        .status(403)
        .json({ success: false, message: "Bạn không thuộc phòng chat này" });
    }
...
exports.sendMessage = async (req, res) => {
  ...
  try {
    ...
    if (!id_phong || !String(noi_dung || "").trim()) {
      return res
        .status(400)
        .json({ success: false, message: "Thiếu nội dung tin nhắn" });
    }
...
exports.sendMediaMessage = async (req, res) => {
  ...
  try {
    ...
    if (!id_phong || !file) {
      return res
        .status(400)
        .json({ success: false, message: "Thiếu tệp media" });
    }
...
exports.createGroupRoom = async (req, res) => {
  ...
  const normalizedMemberIds = Array.from(
    new Set(
      (Array.isArray(thanh_vien_ids) ? thanh_vien_ids : []).map((id) =>
        Number(id),
      ),
    ),
  ...
  try {
    ...
    if (users.length !== normalizedMemberIds.length) {
      return res
        .status(400)
        .json({ success: false, message: "Có thành viên không hợp lệ" });
    }
